...
static void smblib_conn_therm_work(struct work_struct *work)
{
	...
	if (chg->connector_temp_pre - chg->connector_temp > 10 ||
	    chg->connector_temp - chg->connector_temp_pre > 10) {
		smblib_dbg(
			chg, PR_OEM,
			"CONN TEMP thermal_status=%d, chip->thermal_status=%d, connect_temp= %d\n",
			thermal_status, chg->thermal_status,
			chg->connector_temp);
...
static void smblib_charger_type_recheck(struct work_struct *work)
{
	...
	if (chg->typec_port && !chg->pr_swap_in_progress) {
		...
		*/
		...
		queue_delayed_work(
			system_power_efficient_wq, &chg->pr_swap_detach_work,
...
static void smblib_chg_termination_work(struct work_struct *work)
{
	...
	if (rc < 0) {
		...
		rc = smblib_get_prop_from_bms(
			chg, POWER_SUPPLY_PROP_VOLTAGE_MAX_DESIGN, &pval);
		if (rc < 0)
			goto out;
	}
