...
#define UCLAMP_FLAG_IDLE 0x01
	...
	DECLARE_BITMAP_ARRAY(top_tasks_bitmap, NUM_TRACKED_WINDOWS,
			     NUM_LOAD_INDICES);
...
unsigned long cpu_util_freq_walt(int cpu,
...
arch_scale_max_freq_capacity(struct sched_domain *sd, int cpu)
{
	return SCHED_CAPACITY_SCALE;
}
#endif
...
static inline bool task_in_cum_window_demand(struct rq *rq,
					     struct task_struct *p)
{
...
static inline enum sched_boost_policy task_boost_policy(struct task_struct *p)
{
	enum sched_boost_policy policy =
		task_sched_boost(p) ? sched_boost_policy() : SCHED_BOOST_NONE;
...
static inline u32 task_load(struct task_struct *p)
{
	return 0;
}
static inline u32 task_pl(struct task_struct *p)
{
	return 0;
}
