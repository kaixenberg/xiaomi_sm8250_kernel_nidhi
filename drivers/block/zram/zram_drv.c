...
static inline void update_used_max(struct zram *zram, const unsigned long pages)
{
	...
	do {
		if (cur_max >= pages)
			return;
	} while (!atomic_long_try_cmpxchg(&zram->stats.max_used_pages, &cur_max,
					  pages));
}
...
static bool place_pp_slot(struct zram *zram, struct zram_pp_ctl *ctl, u32 index)
{
	struct zram_pp_slot *pps;
	u32 bid;
...
static int scan_slots_for_writeback(struct zram *zram, u32 mode,
				    unsigned long nr_pages, unsigned long index,
				    struct zram_pp_ctl *ctl)
{
...
static void zram_debugfs_register(struct zram *zram)
{
	...
	zram->debugfs_dir =
		debugfs_create_dir(zram->disk->disk_name, zram_debugfs_root);
	debugfs_create_file("block_state", 0400, zram->debugfs_dir, zram,
			     ...
			     const char *dict_path)
{
	...
	if (dict_path) {
		ret = kernel_read_file_from_path(dict_path,
						 &zram->params[prio].dict, &sz,
						 INT_MAX, READING_POLICY);
		if (ret)
			return ret;
	}
...
static int zram_bvec_read(struct zram *zram, struct bio_vec *bvec, u32 index,
...
static int zram_write_page(struct zram *zram, struct page *page, u32 index)
{
	...
	mem = kmap_atomic(page);
	ret = zcomp_compress(zram->comps[ZRAM_PRIMARY_COMP], zstrm, mem,
			     &comp_len);
	kunmap_atomic(mem);
			   ...
			   u32 prio_max)
{
	...
	for (; prio < prio_max; prio++) {
		...
		class_index_new =
			zs_lookup_class_index(zram->mem_pool, comp_len_new);
				...
				size_t len)
{
	...
	while ((pps = select_pp_slot(ctl))) {
		...
		err = recompress_slot(zram, pps->index, page, &num_recomp_pages,
	...
	static void __zram_make_request(struct zram * zram, struct bio * bio)
	{
						...
						struct bio * bio)
		{
			...
			bio_for_each_segment (bv, bio, iter) {
				...
				static void zram_destroy_comps(struct zram *
							       zram)
				{
					u32 prio;

					for (prio = ZRAM_PRIMARY_COMP;
				...
				static void zram_reset_device(struct zram *
							      zram)
				{
					down_write(&zram->init_lock);
					...
					part_stat_set_all(&zram->disk->part0,
							  0);
					...
					const char *buf, size_t len)
				{
					...
					pr_info("Setting zRAM size to %li GB",
						disksize / 1073741824);
 ...
 * Handler function for all zram I/O requests.
 */
				static blk_qc_t zram_make_request(
				...
				static void zram_comp_params_reset(struct zram *
								   zram)
				{
					u32 prio;
				...
				static int zram_open(struct block_device * bdev,
						     fmode_t mode)
				{
					int ret = 0;
					struct zram *zram;
 ...
 */
				static int zram_add(void)
				{
					...
					zram = kzalloc(sizeof(struct zram),
						       GFP_KERNEL);
					if (!zram)
						return -ENOMEM;

					ret = idr_alloc(&zram_index_idr, zram,
							0, 0, GFP_KERNEL);
					if (ret < 0)
						goto out_free_dev;
					device_id = ret;
					...
					snprintf(zram->disk->disk_name, 16,
						 "zram%d", device_id);
					atomic_set(&zram->pp_in_progress, 0);
					zram_comp_params_reset(zram);
					comp_algorithm_set(zram,
	 ...
	 */
					blk_queue_physical_block_size(
						zram->disk->queue, PAGE_SIZE);
					blk_queue_logical_block_size(
						zram->disk->queue,
						ZRAM_LOGICAL_BLOCK_SIZE);
					blk_queue_io_min(zram->disk->queue,
							 PAGE_SIZE);
					blk_queue_io_opt(zram->disk->queue,
							 PAGE_SIZE);
					zram->disk->queue->limits
						.discard_granularity =
						PAGE_SIZE;
	 ...
	 * zeroed.
	 */
					if (ZRAM_LOGICAL_BLOCK_SIZE ==
					    PAGE_SIZE)
						blk_queue_max_write_zeroes_sectors(
							zram->disk->queue,
							UINT_MAX);

					zram->disk->queue->backing_dev_info
						->capabilities |=
 ...
 */
				...
				static struct class_attribute class_attr_hot_add =
					__ATTR(hot_add, 0400, hot_add_show,
					       NULL);

				static ssize_t hot_remove_store(
					struct class * class,
					struct class_attribute * attr,
				...
				static int zram_remove_cb(int id, void *ptr,
							  void *data)
				{
